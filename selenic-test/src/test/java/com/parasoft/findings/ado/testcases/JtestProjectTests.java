...
public class JtestProjectTests {
    ...
    public void testParasoftFindingsPlugin() {
        ...
        GlobalUtils.createProject(driver, Properties.JTEST_PROJECT_NAME);

        GlobalUtils.importRepository(driver, Properties.CLONE_JTEST_PROJECT_URL);

        CreatePipelinesPage createPipelinesPage = new CreatePipelinesPage(driver);
        GlobalUtils.createPipeline(createPipelinesPage);
        GlobalUtils.addCommandLineTask(createPipelinesPage, Properties.GENERATE_STATIC_ANALYSIS_REPORT, Properties.GENERATE_JTEST_STATIC_ANALYSIS_REPORT_COMMAND);
        GlobalUtils.addPublishParasoftResultsTask(createPipelinesPage, Properties.ANALYZE_STATIC_ANALYSIS_REPORT, Properties.JTEST_STATIC_ANALYSIS_RESULTS_FILES_PATH, Properties.SETTINGS_FIELD);
        ...
        assertTrue(pipelineRunPage.getReportTitle().contains(Properties.JTEST_REPORT_TOOL_NAME));
        assertTrue(pipelineRunPage.getReportTitle().contains(Properties.JTEST_NUMBER_OF_RULE_DOCS));
        ...
        assertTrue(ruleDocPage.getJtestRuleText().contains(ruleLinkText));
        ...
        GlobalUtils.deleteProject(driver, Properties.JTEST_PROJECT_NAME);
    }
}