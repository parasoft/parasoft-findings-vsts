...
export class ParaReportPublishService {
    ...
    transformReports = async (inputReportFiles: string[], index: number): Promise<void> => {
        ...
        if (report.toLocaleLowerCase().endsWith(this.SARIF_EXTENSION)) {
            ...
            if (this.pipelineType == PipelineTypeEnum.RELEASE) {
                tl.warning("Parsing static analysis report is not supported in the release pipeline, skip report: " + report);
            } else {
    ...
    transformToSarif = (sourcePath: string): void => {
        if (this.pipelineType == PipelineTypeEnum.RELEASE) {
            tl.warning("Parsing static analysis report is not supported in the release pipeline, skip report: " + sourcePath);
            return;
        }
    ...
    transformToCobertura = (sourcePath: string): void => {
        if (this.pipelineType == PipelineTypeEnum.RELEASE) {
            tl.warning("Parsing code coverage report is not supported in the release pipeline, skip report: " + sourcePath);
            return;
        }
